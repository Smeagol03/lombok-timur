        ...
        <div class="bg-primary py-10 sm:py-14">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <nav class="flex items-center text-sm !text-white mb-4">
                    ...
                    <a href="{{ url('/pengumuman') }}" class="!text-white hover:!text-white/80 transition-colors">Pengumuman</a>
                    <svg class="w-4 h-4 mx-2 !text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    ...
                    <span class="px-3 py-1 text-xs sm:text-sm font-semibold bg-red-500 text-white rounded">
                        Penting
                    </span>
                    @endif
                    <span class="px-3 py-1 text-xs sm:text-sm font-medium bg-white/20 text-white rounded">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sm:p-8 md:p-10">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-amber-800 text-sm">
                                @if($pengumuman->tanggal_kadaluarsa->isFuture())
                                    Berlaku hingga: <strong>{{ $pengumuman->tanggal_kadaluarsa->format('d M Y') }}</strong>
                                @else
                                    <span class="font-semibold">Sudah kadaluarsa sejak {{ $pengumuman->tanggal_kadaluarsa->format('d M Y') }}</span>
                                @endif
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 sm:mt-10 pt-5 sm:pt-6 border-t border-gray-100">
                        ...
                        <div class="space-y-2 sm:space-y-3">
                            ...
                            <a href="{{ $media->getUrl() }}"
                               target="_blank"
                               class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors border border-gray-100 hover:border-gray-200">
                                <svg class="w-7 h-7 sm:w-8 sm:h-8 text-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                </svg>
                                <div class="min-w-0 flex-1">
                                    <p class="font-medium text-gray-900 text-sm sm:text-base truncate">{{ $media->name }}</p>
                                    <p class="text-xs sm:text-sm text-gray-500">{{ number_format($media->size / 1024, 1) }} KB</p>
                                </div>
